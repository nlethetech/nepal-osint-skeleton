...
class TweetDedupService:
    ...
    async def _haiku_batch(
        ...
        tweet_lines = []
        id_map: dict[int, Tweet] = {}
        for idx, tweet in enumerate(tweets[:30], 1):  # Cap at 30 tweets per batch
            id_map[idx] = tweet
            text_preview = tweet.text[:280].replace('\n', ' ')
            tweet_lines.append(f"[{idx}] {text_preview}")

        tweet_block = "\n".join(tweet_lines)

        prompt = f"""You are analyzing Nepal OSINT tweets. Do TWO things:

1. CLUSTER: Group tweets about the same event/topic. Return cluster IDs.
   - Cross-language grouping: Nepali and English tweets about the same event should be in the same cluster.
   - Only group tweets that are clearly about the SAME specific event, not just the same broad topic.
2. LOCATIONS: Extract Nepal locations mentioned (districts, cities, landmarks).
   - Map city names to their district (e.g., "Pokhara" → "Kaski", "Biratnagar" → "Morang").
   - Only extract Nepal locations, ignore foreign locations.

TWEETS:
{tweet_block}

Respond ONLY with JSON (no markdown fences):
{{"clusters": [[1,2], [3,4], [5]], "locations": {{"1": ["Kailali"], "2": ["Kailali"], "3": [], "4": [], "5": ["Kathmandu"]}}}}"""

        result = await call_claude_json(prompt, timeout=60, model="haiku")
