...
async def list_bills(
    ...
    items = [
        BillResponse(
            ...
            presented_by=bill.presented_by,
            ai_analysis=bill.ai_analysis,
        )
        for bill in bills
    ]
...
async def list_questions(
    ...
    question_type: Optional[str] = Query(None, description="oral or written"),
    questioner: Optional[str] = Query(None, description="Filter by questioner name"),
    answered_only: bool = Query(False, description="Only answered questions"),
    page: int = Query(1, ge=1),
    page_size: int = Query(50, ge=1, le=200),
    db: AsyncSession = Depends(get_db),
):
    """List parliamentary questions from the 6th session Q&A records."""
    from app.models.parliament import ParliamentQuestion
    from sqlalchemy import select, func

    query = select(ParliamentQuestion)

    if ministry:
        query = query.where(ParliamentQuestion.ministry_addressed.ilike(f"%{ministry}%"))
    if question_type:
        query = query.where(ParliamentQuestion.question_type == question_type)
    if questioner:
        query = query.where(
            ParliamentQuestion.questioner_name.ilike(f"%{questioner}%")
        )
    if answered_only:
        query = query.where(ParliamentQuestion.answered == True)
    ...
    # Fetch page
    query = (
        query.order_by(ParliamentQuestion.external_id.asc())
...
async def questions_summary(
    ...
    """Summary statistics for parliamentary questions."""
    from app.models.parliament import ParliamentQuestion
    from sqlalchemy import select, func
    ...
    answered = (await db.execute(
        select(func.count(ParliamentQuestion.id))
        .where(ParliamentQuestion.answered == True)
    )).scalar() or 0
    ...
    # By ministry
    ministry_result = await db.execute(
        select(ParliamentQuestion.ministry_addressed, func.count(ParliamentQuestion.id))
        .where(ParliamentQuestion.ministry_addressed.isnot(None))
        .group_by(ParliamentQuestion.ministry_addressed)
        .order_by(func.count(ParliamentQuestion.id).desc())
    )
    by_ministry = [
        {"ministry": row[0], "count": row[1]}
        for row in ministry_result.all()
    ]
...
MINISTRY_EN: dict[str, tuple[str, str]] = {
    # Nepali name -> (English name, typical question themes)
    "उद्योग, वाणिज्य तथा आपूर्ति": (
        "Ministry of Industry, Commerce & Supply",
        "Industrial policy, trade regulation, consumer protection, supply chain issues",
    ),
    "गृह मन्त्रालय": (
        "Ministry of Home Affairs",
        "Law enforcement, internal security, border management, public safety",
    ),
    "ऊर्जा, जलस्रोत तथा सिँचाई": (
        "Ministry of Energy, Water Resources & Irrigation",
        "Hydropower development, irrigation projects, water management, energy policy",
    ),
    "कृषि तथा पशुपन्छी विकास": (
        "Ministry of Agriculture & Livestock Development",
        "Agricultural subsidies, food security, livestock programs, farmer support",
    ),
    "वन तथा वातावरण": (
        "Ministry of Forests & Environment",
        "Forest conservation, environmental protection, national parks, climate action",
    ),
    "रक्षा मन्त्रालय": (
        "Ministry of Labour, Employment & Social Security",
        "Foreign employment, labor rights, migrant worker welfare, insurance claims",
    ),
    "अर्थ मन्त्रालय": (
        "Ministry of Finance",
        "Budget allocation, taxation, fiscal policy, economic planning",
    ),
    "भौतिक पूर्वाधार तथा यातायात": (
        "Ministry of Physical Infrastructure & Transport",
        "Road construction, transport regulation, infrastructure projects, connectivity",
    ),
    "सञ्चार तथा सूचना प्रविधि": (
        "Ministry of Communication & Information Technology",
        "Telecom policy, digital infrastructure, media regulation, IT development",
    ),
    "शिक्षा, विज्ञान तथा प्रविधि": (
        "Ministry of Education, Science & Technology",
        "Education reform, school infrastructure, research funding, curriculum policy",
    ),
    "शहरी विकास": (
        "Ministry of Urban Development",
        "Urban planning, housing policy, municipal infrastructure, corridor development",
    ),
    "भूमि व्यवस्था, सहकारी तथा गरिबी निवारण": (
        "Ministry of Land Management, Cooperatives & Poverty Alleviation",
        "Land reform, cooperative regulation, poverty reduction programs",
    ),
    "परराष्ट्र मन्त्रालय": (
        "Ministry of Foreign Affairs",
        "Diplomatic relations, consular services, international treaties, border issues",
    ),
    "युवा तथा खेलकुद": (
        "Ministry of Youth & Sports",
        "Youth employment, sports infrastructure, skill development programs",
    ),
    "संस्कृति, पर्यटन तथा नागरिक उड्डयन": (
        "Ministry of Culture, Tourism & Civil Aviation",
        "Tourism promotion, cultural heritage, aviation infrastructure, airports",
    ),
    "प्रधानमन्त्री तथा मन्त्रिपरिषद्को कार्यालय": (
        "Office of the Prime Minister & Council of Ministers",
        "Executive policy, national priorities, inter-ministerial coordination",
    ),
    "महिला, बालबालिका तथा ज्येष्ठ नागरिक": (
        "Ministry of Women, Children & Senior Citizens",
        "Gender equality, child protection, elderly welfare, social programs",
    ),
    "स्वास्थ्य तथा जनसंख्या": (
        "Ministry of Health & Population",
        "Healthcare access, public health policy, hospital infrastructure, epidemics",
    ),
    "कानून, न्याय तथा संसदीय मामिला": (
        "Ministry of Law, Justice & Parliamentary Affairs",
        "Judicial reform, legal aid, parliamentary procedure, constitutional issues",
    ),
    "खानेपानी मन्त्रालय": (
        "Ministry of Water Supply",
        "Drinking water access, sanitation, water treatment, rural water supply",
    ),
    "संघीय मामिला तथा सामान्य प्रशासन": (
        "Ministry of Federal Affairs & General Administration",
        "Federalism implementation, civil service, local government coordination",
    ),
}

# Transliteration map for questioner names
QUESTIONER_EN: dict[str, str] = {
    "माधव सापकोटा": "Madhav Sapkota",
    "सोविता गौतम": "Sovita Gautam",
    "पूर्ण बहादुर घर्ति": "Purna Bahadur Gharti",
    "सूर्य प्रसाद ढकाल": "Surya Prasad Dhakal",
    "चन्दा कार्की": "Chanda Karki",
    "प्रेम सुवाल": "Prem Suwal",
    "सराज अहमद फारुकी": "Saraj Ahmad Faruqi",
    "रोशन कार्की": "Roshan Karki",
    "नारायणी शर्मा": "Narayani Sharma",
    "शिशिर नेपाल": "Shishir Nepal",
    "मेटमणी चौधरी": "Metmani Chaudhary",
    "रामुलस भट्टराई": "Ramulus Bhattarai",
}


@router.get("/questions/as-sessions")
async def questions_as_sessions(
    db: AsyncSession = Depends(get_db),
):
    """Return all Q&A data as a single session with ministry-level agenda items."""
    from app.models.parliament import ParliamentQuestion
    ...
    result = await db.execute(
        ...
        .order_by(ParliamentQuestion.ministry_addressed, ParliamentQuestion.external_id)
    )
    all_questions = result.scalars().all()

    if not all_questions:
        return {"sessions": [], "total_questions": 0}

    total_q = len(all_questions)
    answered_count = sum(1 for q in all_questions if q.answered)
    all_questioners_ne = list(set(q.questioner_name for q in all_questions if q.questioner_name))
    all_questioners_en = [QUESTIONER_EN.get(n, n) for n in all_questioners_ne]

    # Group by ministry
    by_ministry: dict[str, list] = {}
    for q in all_questions:
        ministry = q.ministry_addressed or "Unknown"
        by_ministry.setdefault(ministry, []).append(q)

    agenda_items = []
    key_topics_en = []
    for ministry_ne, qs in sorted(by_ministry.items(), key=lambda x: -len(x[1])):
        m_answered = sum(1 for q in qs if q.answered)
        m_total = len(qs)
        m_questioners_ne = list(set(q.questioner_name for q in qs if q.questioner_name))
        m_questioners_en = [QUESTIONER_EN.get(n, n) for n in m_questioners_ne]

        en_name, themes = MINISTRY_EN.get(ministry_ne, (ministry_ne, "Various policy questions"))
        key_topics_en.append(en_name)

        answer_rate = f"{m_answered}/{m_total} answered" if m_answered else f"0/{m_total} answered"
        desc = f"{themes}. Asked by {', '.join(m_questioners_en[:3])}"
        if len(m_questioners_en) > 3:
            desc += f" and {len(m_questioners_en) - 3} others"
        desc += "."

        agenda_items.append({
            "topic": en_name,
            "intensity": "high" if m_total >= 15 else "medium" if m_total >= 8 else "low",
            "outcome": answer_rate,
            "description": desc,
            "supporters": m_questioners_en[:5],
        })

    top2 = [MINISTRY_EN.get(m, (m, ""))[0] for m, _ in
            sorted(by_ministry.items(), key=lambda x: -len(x[1]))[:2]]

    summary = (
        f"{total_q} parliamentary questions were raised by {len(all_questioners_en)} MPs "
        f"across {len(by_ministry)} ministries during the dissolved 6th Session of the "
        f"House of Representatives. {answered_count} questions received ministerial responses "
        f"while {total_q - answered_count} remain unanswered. "
        f"The most questioned ministries were {top2[0]} and {top2[1]}. "
        f"Key questioners include {', '.join(all_questioners_en[:4])}."
    )

    session = {
        "id": hashlib.md5(b"hor6-qa-session").hexdigest(),
        "title_ne": "6th Session Q&A (Dissolved HoR)",
        "session_date": None,
        "session_date_bs": "2082/01/12 – 2082/05/27",
        "meeting_no": None,
        "speech_count": total_q,
        "session_summary": summary,
        "key_topics": key_topics_en[:6],
        ...
        "questioners": all_questioners_en,
    }

    return {
        "sessions": [session],
        "total_questions": total_q,
        "total_ministries": len(by_ministry),
    }
...
class QuestionIngestItem(PydanticBaseModel):
    external_id: str
    ...
    answered: bool = False
    question_date_bs: Optional[str] = None
    question_number: Optional[str] = None
    session: Optional[str] = None


@router.post("/questions/ingest", dependencies=[Depends(require_dev)])
async def ingest_questions(
    payload: list[QuestionIngestItem],
    db: AsyncSession = Depends(get_db),
):
    """Ingest parliamentary questions from extraction script (dev-only)."""
    from app.models.parliament import ParliamentQuestion
    from sqlalchemy import select
    ...
    for item in payload:
        result = await db.execute(
            select(ParliamentQuestion).where(
                ParliamentQuestion.external_id == item.external_id
            )
        )
        existing = result.scalar_one_or_none()

        if existing:
            existing.question_text = item.question_text or existing.question_text
            existing.ministry_addressed = item.ministry or existing.ministry_addressed
            existing.question_type = item.question_type or existing.question_type
            existing.answered = item.answered
            existing.questioner_name = item.questioner or existing.questioner_name
            existing.answer_text = item.answer_text or existing.answer_text
            existing.session_label = item.session or existing.session_label
            updated += 1
        else:
            q = ParliamentQuestion(
                id=uuid4(),
                external_id=item.external_id,
                question_type=item.question_type,
                question_text=item.question_text,
                ministry_addressed=item.ministry,
                answered=item.answered,
                questioner_name=item.questioner,
    ...
    await db.commit()
    return {"created": created, "updated": updated, "total": len(payload)}


class BillAnalysisIngest(PydanticBaseModel):
    external_id: str
...
@router.post("/bills/ingest-analysis", tags=["parliament-admin"])
async def ingest_bill_analysis(
    payload: list[BillAnalysisIngest],
    db: AsyncSession = Depends(get_db),
    user=Depends(require_dev),
):
    """Ingest OCR text and AI analysis for bills (dev-only, used by local runner)."""
    ...
    for item in payload:
        result = await db.execute(
            select(ParliamentBill).where(ParliamentBill.external_id == item.external_id)
        )
        bill = result.scalar()
        if not bill:
            not_found.append(item.external_id)
            continue
        if item.ocr_text:
            bill.ocr_text = item.ocr_text
        if item.ai_analysis:
            bill.ai_analysis = item.ai_analysis
        if item.presented_by:
            bill.presented_by = item.presented_by
        if item.pdf_url:
